...
index 44b698b..01e6057 100644
--- a/crates/rust/src/lib.rs
+++ b/crates/rust/src/lib.rs
@@ -1362,7 +1362,8 @@ impl Bindgen for FunctionBindgen<'_, '_> {
                 self.push_str(&format!("let {} = {}.as_ptr() as i32;\n", ptr, val));
                 self.push_str(&format!("let {} = {}.len() as i32;\n", len, val));
                 if realloc.is_some() {
-                    self.push_str(&format!("core::mem::forget({});\n", val));
+                    self.push_str("extern crate core as __core;\n");
+                    self.push_str(&format!("__core::mem::forget({});\n", val));
                 }
                 results.push(ptr);
                 results.push(len);
```
